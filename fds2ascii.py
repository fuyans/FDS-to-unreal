...
if __name__ == "__main__":
    ...
    fp_smv = askopenfilename(title='Select a Smokeview file', filetypes=[('Smokeview', '.smv')])
    start = 10
    end = 20
    dir_out = askdirectory(title='Select output directory')

    fp_smv = path.realpath(fp_smv)
    # 1 - Get the grid info
    _x, _y, _z = getGridFromSMV(fp_smv)

    # 2 - Get the output data files (.s3d and .sf)
    files = getOutputFilesFromSMV(fp_smv)
    ...
    for f in files:
        # Read the .s3d files
        if f[1][-4:] == ".s3d":
            with open(path.join(path.dirname(fp_smv), f[1] + ".sz")) as fsz:
                frames = np.array([[float(x) for x in l.split()] for l in fsz.readlines()[1:]])
            if start > len(frames):
                print(str(start) + " > nFrames (" + str(len(frames)) + ")!")
                sys.exit()
            if end != 0:
                if end < start:
                    ...
                    sys.exit()
                if end > len(frames):
                    print(str(end) + " > nFrames (" + str(len(frames)) + ")!")
                    sys.exit()
            else:
                end = len(frames)
            #
            values = readS3D(path.join(path.dirname(fp_smv), f[1]), frames, start, end)
            print(len(values))
            #
            for i, v in enumerate(values):
                vtkFile = path.join(dir_out, path.basename(fp_smv) + "_" + f[0] + "_" + str(start + i + 1) + ".vtk")
